...
def make_train(config):
    ...
    def train(rng):
        ...
        """
        Use different rng for each env!
        In the case of meta-cartpole, we use indep random projs for each env.
        """
        ...
        # TRAIN LOOP
        def _update_step(runner_state, unused,update_network=True):
        ...
        runner_state, metric = jax.lax.scan(_update_step, runner_state, None, True, config["NUM_UPDATES"])
        #runner_state, metric = jax.lax.scan(_update_step, runner_state, None, True, config["NUM_META_TRAIN"])
        #runner_state, metric = jax.lax.scan(_update_step, runner_state, None, False, config["NUM_META_EVAL"])
